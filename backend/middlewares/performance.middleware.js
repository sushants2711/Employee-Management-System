import joi from "joi";
import { internalServerErrorResponse, validationErrorResponse } from "../utils/response.handler";
...
export const createPerformaceMiddleware = async (req, res, next) => {
    try {
        const schema = joi.object({
            ...
            status: joi.string().valid("EXCELLENT", "GOOD", "AVERAGE", "NEEDS_IMPROVEMENT", "POOR").required(),
...
export const updatePerformanceMiddleware = async (req, res, next) => {
    try {
        const schema = joi.object({
            ...
            status: joi.string().valid("EXCELLENT", "GOOD", "AVERAGE", "NEEDS_IMPROVEMENT", "POOR").optional().allow(""),
            remarks: joi.string().optional().allow(""),
            strengths: joi.string().optional().allow(""),
