...
export const createProjectController = async (req, res) => {
    try {
        const { projectName, description, teamName, manager, teamLeader, startDate, endDate, members } = req.body;

        const loggedInUser = req.user.id;

        if (!loggedInUser) {
            return unauthorizedResponse(res, "Unauthorized", "You are not authorized to perform this action");
        }
        ...
        if (teamExist.teamLead !== teamLeader) {
            return badRequestResponse(res, "Team Leader is not a team leader of this team");
        }
        ...
        const activeMembers = membersExist.filter(member => member.status === "ACTIVE");
...
export const getAllProjectController = async (req, res) => {
    try {
        const { status, teamName, manager, teamLeader, member, assignBy, isActive, startDate, endDate, projectName } = req.query;
...
export const updateProjectController = async (req, res) => {
    try {
        ...
        const { projectName, description, teamName, manager, teamLeader, startDate, endDate, members, status, assignBy } = req.body;
