...
import { showSuccess, showError, showInfo } from "../toastMessage/toastDeliver";
...
function Projects() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setIsLoading(true);
      const projectsRes = await getAllProjects(statusFilter, searchQuery);
  ...
  const handleInputChange = (e) => {
    ...
    const error = validateProjectField(name, value);
    setErrors((prev) => ({
      ...prev,
      [name]: error,
    }));
  ...
  const fetchAvailableTeams = async (projectId = null) => {
    try {
      const res = await getAvailableTeams(projectId);
      setTeams(res.data || []);
    } catch (error) {
      showError(error.message || "Failed to fetch available teams");
      setTeams([]);
    }
  };
  ...
  const openCreateModal = async () => {
    ...
    setModalConfig({ isOpen: true, mode: "CREATE", projectId: null });
    setFormData({
      projectName: "",
      description: "",
      teamName: "",
  ...
  const openViewModal = (project) => {
    setModalConfig({ isOpen: true, mode: "VIEW", projectId: project._id });
    setSelectedItem(project);
    setErrors({});
  };

  const openUpdateModal = async (project) => {
    await fetchAvailableTeams(project._id);
    setModalConfig({ isOpen: true, mode: "UPDATE", projectId: project._id });
    setFormData({
      projectName: project.projectName,
      description: project.description || "",
      teamName: project.teamName?._id || project.teamName,
  ...
  const handleSubmit = async (e) => {
    ...
    const { isValid, errors: formErrors } = validateProjectForm({
      ...formData,
    ...
    // Check for "no changes" in UPDATE mode
    if (modalConfig.mode === "UPDATE" && selectedItem) {
      const hasChanges =
        formData.projectName !== selectedItem.projectName ||
        formData.description !== selectedItem.description ||
        formData.status !== selectedItem.status ||
        formData.startDate !== selectedItem.startDate?.split("T")[0] ||
        formData.endDate !== selectedItem.endDate?.split("T")[0];

      if (!hasChanges) {
        showInfo("No changes were made to the project.");
        setModalConfig({ isOpen: false, mode: "CREATE", projectId: null });
        return;
      }
    }
    ...
    try {
      ...
      if (modalConfig.mode === "CREATE") {
        delete submitData.status; // For project creation, let's keep it simple.
        await createProject(submitData);
        showSuccess("Project created successfully");
      } else {
        delete submitData.teamName; // Team cannot be updated
        await updateProject(modalConfig.projectId, submitData);
      ...
      showError(
        error.message || `Failed to ${modalConfig.mode.toLowerCase()} project`
      );
    } finally {
