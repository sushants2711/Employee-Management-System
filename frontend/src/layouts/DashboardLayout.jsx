...
import { Circle, Sun, Moon, Menu } from "lucide-react";
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative min-w-0">
        <header className="h-16 flex items-center justify-between px-4 sm:px-6 border-b border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm z-10 gap-4">
          <div className="flex items-center">
            {/* Hamburger Menu (Mobile Only) */}
            <button
              onClick={() => setIsSidebarOpen(true)}
              className="md:hidden p-2 mr-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors cursor-pointer"
          ...
          <div className="flex items-center gap-2 sm:gap-4 ml-auto">
            <button
              ...
              className="p-2 rounded-full text-slate-600 dark:text-yellow-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer"
                ...
                <div className="flex items-center gap-2">
                  <Circle
                    className={`w-3.5 h-3.5 fill-current ${
                      STATUS_OPTIONS.find(
                        (opt) =>
                          opt.value === (user?.isAvailable || "Available")
                      )?.color || "text-slate-500"
                    }`}
                  />
